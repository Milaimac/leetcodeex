...
// 如果说是奇数 那么遍历到中位数就ok了， 如果说是偶数， 还需要进行相加除2
    ...
    // 要找到k 个元素， 可以比较A[k/2-1] 和 B[k/2-1]， 由于A[k/2-1]和B[k/2-1] 的前面分别有A[0 .. k/2-2] 和 B[0 .. k/2-2] ，
    // 即k/2-1个元素， 对于A[k/2 - 1] 和 B[k/2 -1] 中的较小值， 最多只有(k/2 - 1) + (k/2 - 1) <= k-2 元素比它小，那么它就不能是第k小的数了
    // 由此归纳出三种情况
    // 1. 如果A[k/2 - 1] < B[k/2 - 1]， 则比A[k/2 -1]小的数最多只有A的前k/2-1个数和B的前k/2-1个数， 但是比A[k/2-1]小的数一定有A数组中的数， 所以排除掉A中的数
    //  即比A[k/2-1]小的数只有k-2个， 所以A[0] 到 A[k/2-1]也不可能是k个数， 可以全部排除

    // 2. 如果A[k/2-1] > B[k/2-1], 就可以排除掉B[0] 和 B[k/2-1]， 那就是给k减去k/2 

    // 3 . 如果 A[k/2 -1] == B[k/2-1] 同第一种情况

    // 三种情况需要特殊处理
    // 如果A[k/2-1] 或者 B[k/2-1] 越界， 那么我们选取对于数组的最后一个元素， 在这种情况下， 必须根据排除数的个数来减少k值， 而不是 直接删除K-2
    // 如果数组为空，说明该数组中的所有元素都被排除， 我们直接返回另一个数组中第k小的元素
    // 如果k =1 ， 只要返回两个数组首元素的最小值即可  
    double findMedianSortedArraysV1(vector<int>& nums1, vector<int>& nums2) {
        int total = nums1.size() + nums2.size();
        if(total% 2 == 1)
        {
            return getKElement(nums1, nums2, (total+1)/2);
        }
        else
        {
    ...
    int getKElement(vector<int>& nums1, vector<int>& nums2, int k)
    {
        ...
        while(true)
        {
            if(index1 == m)
            {
                //已经遍历完nums1, 所以这里直接返回另一个数组中的那个中位数所在的位置
